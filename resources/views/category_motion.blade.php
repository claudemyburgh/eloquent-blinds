            ...
            @if($category->products->count())
                <div class="grid grid-cols-12 gap-6 my-12">
                    ...
                    @foreach($category->products as $product)
                        <x-product-card class="col-span-6 md:col-span-4 lg:col-span-3" :$product route="{{ route('product', [$category, $product]) }}"/>
                    @endforeach
                </div>
            @endif
                ...
                <div class="grid grid-cols-12 gap-6 my-12">
                    ...
                    @foreach($descendants->descendants as $descendant)
                        @foreach($descendant->products as $product)
                            <x-product-card class="col-span-6 md:col-span-4 lg:col-span-3" :$product route="{{ route('product', [$descendant, $product]) }}"/>
                        @endforeach
                    @endforeach
                </div>
            @endif
