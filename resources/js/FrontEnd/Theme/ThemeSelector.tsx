...
export function ThemeSelector(props: any) {
    ...
    return (
        ...
        >
            ...
            <Listbox.Button
                className="flex h-6 w-6 items-center justify-center rounded-md shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-gray-700 dark:ring-inset dark:ring-white/5"
            ...
            <Listbox.Options className="absolute top-full left-1/2 mt-3 w-36 -translate-x-1/2 space-y-1 rounded-lg  p-3 text-sm font-medium shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-gray-900/60 bg-gray-200/60 dark:ring-white/5">
                {themes.map((theme) => (
                    <Listbox.Option
                        key={theme.value}
                        value={theme}
                        className={({ active, selected }) =>
                            twJoin(
                                "flex cursor-pointer select-none items-center rounded-sm p-1",
                                {
                                    // @ts-expect-error
                    ...
                    >
                        {({ selected }) => (
                            <>
                                ...
                                <div className="ml-3 hover:text-primary-400">
                                    {theme.name}
                                </div>
                            </>
                        )}
