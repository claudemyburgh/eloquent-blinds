...
export function ThemeSelector(props: any) {
  ...
  return (
    <Listbox as="div" value={selectedTheme} onChange={setSelectedTheme} {...props}>
      ...
      <Listbox.Button
        className="flex h-10 w-10 items-center justify-center rounded-md shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-gray-800 dark:ring-inset dark:ring-white/5"
      ...
      <Listbox.Options className="absolute top-full left-1/2 mt-3 w-36 -translate-x-1/2 space-y-1 rounded-lg  p-3 text-sm font-medium shadow-md backdrop-blur-lg shadow-black/5 ring-1 ring-black/5 dark:bg-gray-900/95 bg-gray-200/95 dark:ring-white/5">
        {themes.map((theme) => (
          <Listbox.Option
            key={theme.value}
            value={theme}
