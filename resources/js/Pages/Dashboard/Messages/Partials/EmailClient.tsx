...
import { InputError, InputLabel, SaveSubmitButton, Textarea } from "@/Components/Form"
...
const EmailClient = ({ user, subject, parent, ...propsAll }: any) => {
  ...
  const { data, setData, processing, recentlySuccessful, errors, clearErrors, reset, post } = useForm({
    name: user.name,
    email: user.email,
    parent_id: parent,
    subject,
    ...
    <Panel {...propsAll}>
      <form className={`space-y-6`} onSubmit={handleMessageFormInput}>
        <div>
          ...
          <Textarea id="message" name="message" value={data.message} onChange={handleInputUpdate} className="mt-1 block w-full min-h-[300px] disabled:opacity-50" />
