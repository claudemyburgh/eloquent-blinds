...
const Show = ({ auth, message }: any) => {
    ...
    const { data, setData, reset, patch, errors, clearErrors, processing } =
        useForm<{
            read_at: null;
        }>({
            read_at: null,
                ...
                <div className="wrapper ">
                    <Panel>
                        <Panel.Header
                            ...
                            paragraph={`Message from ${message.user.name}`}
                        />
                        <div className="flex justify-between items-center">
                            ...
                            <div>
                                <span
                                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs md:text-sm font-medium bg-gray-100 text-gray-800`}
                                >
                                    Send{" "}
                                    {formatDistance(
                    ...
                    {message.children &&
                        (message.children as unknown as any[]).map((child) => (
                            ...
                            >
                                <Panel.Header
                                    heading={child.subject}
                                    paragraph={`Message from ${child.user.name}`}
                                />
                                <span
                                    className={`inline-flex items-center my-4 px-2.5 py-0.5 rounded-full text-xs md:text-sm font-medium bg-gray-100 text-gray-800`}
                                >
                                    Send{" "}
                                    {formatDistance(
