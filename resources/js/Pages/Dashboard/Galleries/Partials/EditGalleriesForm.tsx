...
const EditGalleriesForm = () => {
  const { gallery, auth } = usePage<GalleryProps & PageProps>().props
  ...
  const handleFormSubmit = (e: any) => {
    ...
    put(route("dashboard.galleries.update", gallery), {
      ...
      preserveScroll: true,
      onSuccess: () => {
        toast.custom((t) => <ToastItem t={t} type={`success`} title={`Success`} message={`Gallery successfully updated`} icon={`check`} />)
      },
      onError: () => {
    ...
    <form className={`mt-6 space-y-6`} onSubmit={handleFormSubmit}>
      ...
      <div>
        ...
        <div className={`flex justify-between items-center`}>
          <SelectInput id={`live`} name={`live`} value={data.live} onChange={handleFormInput}>
...
export default EditGalleriesForm
